...
public class BouncingEnemyAI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        moveSpeed = 1f + ((float)WaveBasedEnemySpawner.currentWaveIndex * 0.1f);
        bounceForce = 3f + ((float)WaveBasedEnemySpawner.currentWaveIndex * 0.2f);
    }
    ...
    void Update()
    {
        ...
        if (turretTransform != null)
        {
            ...
            // Only adjust the x-velocity, preserving the y-velocity (initial launch/bounce velocity)
            rb.velocity = new Vector2(rb.velocity.x + direction.x * moveSpeed * Time.deltaTime, rb.velocity.y);
        }
